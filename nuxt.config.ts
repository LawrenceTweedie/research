...
export default defineNuxtConfig({
  ...
  nitro: {
    ...
    prerender: {
      ...
      concurrency: 1, // Генерируем последовательно для отладки
  ...
  hooks: {
    'nitro:config'(nitroConfig) {
      try {
        ...
        // ВРЕМЕННО: Генерируем маршруты только для первых 3 рынков для тестирования
        const entries = Object.entries(marketsData).slice(0, 3)
        for (const [marketName, marketId] of entries) {
          // Валидация данных
        ...
        // Логируем первые 10 региональных маршрутов для проверки
        const sampleRegionalRoutes = routes.filter(r => r.match(/\/\d+\/\d+$/)).slice(0, 10)
        console.log('📝 Примеры региональных маршрутов:', sampleRegionalRoutes.join(', '))
