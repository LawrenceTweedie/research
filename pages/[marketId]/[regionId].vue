  ...
  <section class="market-info">
    <div class="container market-info__container">
      <div class="market-info__header">
        <h2 class="market-info__title title">{{ marketData?.marketName || 'Загрузка...' }} <span v-if="regionName !== 'Вся Россия'">- {{ regionName }}</span></h2>
...
const { data: response } = await useAsyncData(
  ...
  () => $fetch(`/api/market/${marketId.value}/${regionId.value}`)
)
