...
const region = computed(() => {
  if (!regionsData.value || !regionId.value) return null
  return regionsData.value.find(r => r[0] === regionId.value)
})

const regionName = computed(() => region.value ? region.value[1] : 'Вся Россия')

const activities = computed(() => activitiesData.value || [])

// Компании для этого региона
const companies = computed(() => {
  if (!regionsTop10Data.value || !regionName.value) return []
  return regionsTop10Data.value[regionName.value] || []
})

// Метрики для этого региона
const metrics = computed(() => {
  if (!regionDataAll.value || !regionName.value) return {}
  return regionDataAll.value[regionName.value] || {}
})
...
useHead({
  title: `${marketName.value} в ${regionName.value} - Анализ рынка | INRESEARCH`,
  meta: [
    {
      name: 'description',
      content: `Анализ рынка "${marketName.value}" в регионе ${regionName.value}. Объем рынка: ${marketVolume.value}, рентабельность: ${profitability.value}. Топ-10 компаний региона, финансовые показатели за 2020-2024 годы.`
    },
    {
      name: 'keywords',
      content: `${marketName.value}, ${regionName.value}, анализ рынка, объем рынка, рентабельность, топ компании, финансовые показатели`
    },
    { property: 'og:title', content: `${marketName.value} в ${regionName.value} - Анализ рынка` },
    { property: 'og:description', content: `Объем рынка: ${marketVolume.value}, рентабельность: ${profitability.value}. Топ-10 компаний региона и финансовые показатели.` },
...
const selectedRegionName = ref(regionName.value)
